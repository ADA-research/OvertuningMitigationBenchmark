"""
Bagging Classifier Space Configuration.

This module defines the `BaggingSpace` class, which is responsible
for constructing the hyperparameter search space for the BaggingClassifier
model using ConfigSpace. All hyperparameters added to this space
are conditional on the model being set to 'BaggingClassifier'.

Classes:
    - BaggingSpace: Constructs the hyperparameter space for BaggingClassifier.
"""
...
class BaggingSpace(BaseComponentSpace):
    """
    Defines the configuration space for the BaggingClassifier model.

    This class builds the search space for fitting a BaggingClassifier model
    ...
    def __init__(self, component_hp, seed=0):
        """
        Initializes the BaggingClassifier hyperparameter search space.

        Args:
            component_hp (ConfigSpace.hyperparameter.Hyperparameter):
                The top-level hyperparameter selector that identifies the model
                type (e.g., 'BaggingClassifier').
        """
        super().__init__(component_hp, name="Bagging", seed=seed)

        # Add hyperparameters specific to BaggingClassifier
