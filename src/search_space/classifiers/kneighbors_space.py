"""
KNeighbors Classifier Space Configuration.

This module defines the `KNeighborsSpace` class, which is responsible
for constructing the hyperparameter search space for the KNeighborsClassifier
model using ConfigSpace. All hyperparameters added to this space
are conditional on the model being set to 'KNeighborsClassifier'.

Classes:
    - KNeighborsSpace: Constructs the hyperparameter space for KNeighborsClassifier.
"""
...
class KNeighborsSpace(BaseComponentSpace):
    """
    Defines the configuration space for the KNeighborsClassifier model.

    This class builds the search space for fitting a KNeighborsClassifier model
    ...
    def __init__(self, component_hp, seed=0):
        """
        Initializes the KNeighborsClassifier hyperparameter search space.

        Args:
            component_hp (ConfigSpace.hyperparameter.Hyperparameter):
                The top-level hyperparameter selector that identifies the model
                type (e.g., 'KNeighborsClassifier').
        """
        super().__init__(component_hp, name="KNeighbors", seed=seed)

        # Add hyperparameters specific to KNeighborsClassifier
