"""
Decision Tree Classifier Space Configuration.

This module defines the `DecisionTreeSpace` class, which is responsible
for constructing the hyperparameter search space for the DecisionTreeClassifier
model using ConfigSpace. All hyperparameters added to this space
are conditional on the model being set to 'DecisionTreeClassifier'.

Classes:
    - DecisionTreeSpace: Constructs the hyperparameter space for DecisionTreeClassifier.
"""
...
class DecisionTreeSpace(BaseComponentSpace):
    """
    Defines the configuration space for the DecisionTreeClassifier model.

    This class builds the search space for fitting a DecisionTreeClassifier model
    ...
    def __init__(self, component_hp, seed=0):
        """
        Initializes the DecisionTreeClassifier hyperparameter search space.

        Args:
            component_hp (ConfigSpace.hyperparameter.Hyperparameter):
                The top-level hyperparameter selector that identifies the model
                type (e.g., 'DecisionTreeClassifier').
        """
        super().__init__(component_hp, name="DecisionTree", seed=seed)

        # Add hyperparameters specific to DecisionTreeClassifier
        self.add_hyperparameters((
            ...
            ConfigSpace.CategoricalHyperparameter(
                f"{self.name}_criterion", choices=["gini", "entropy", "log_loss"], default_value="gini"
            ),
            ConfigSpace.CategoricalHyperparameter(
