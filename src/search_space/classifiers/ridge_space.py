"""
Ridge Classifier Space Configuration.

This module defines the `RidgeSpace` class, which is responsible
for constructing the hyperparameter search space for the RidgeClassifier
model using ConfigSpace. All hyperparameters added to this space
are conditional on the model being set to 'RidgeClassifier'.

Classes:
    - RidgeSpace: Constructs the hyperparameter space for RidgeClassifier.
"""
...
class RidgeSpace(BaseComponentSpace):
    """
    Defines the configuration space for the RidgeClassifier model.

    This class builds the search space for fitting a RidgeClassifier model
    via ConfigSpace. It includes hyperparameters such as regularization strength
    and solver choice. Additional control is provided for fine-tuning the classifier.
    ...
    def __init__(self, component_hp, seed=0):
        """
        Initializes the RidgeClassifier hyperparameter search space.

        Args:
            component_hp (ConfigSpace.hyperparameter.Hyperparameter):
                The top-level hyperparameter selector that identifies the model
                type (e.g., 'RidgeClassifier').
        """
        super().__init__(component_hp, name="Ridge", seed=seed)

        # Add hyperparameters specific to RidgeClassifier
