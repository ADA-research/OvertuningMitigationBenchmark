...
class LGBMSpace(BaseComponentSpace):
    ...
    def __init__(self, component_hp, seed=0):
        ...
        self.add_hyperparameters((
            ...
            # ConfigSpace.Constant(
            #     f"{self.name}_early_stopping_rounds", 300
            # )
        ))
        ...
        # OLD SEARCH SPACE FOR LGBM
        # self.add_hyperparameters((
        #     ConfigSpace.UniformIntegerHyperparameter(
        #         f"{self.name}_n_estimators", 50, 1000, default_value=100
        #     ),
        #     ConfigSpace.UniformFloatHyperparameter(
        #         f"{self.name}_learning_rate", 0.005, 0.1, default_value=0.01, log=True
        #     ),
        #     ConfigSpace.UniformIntegerHyperparameter(
        #         f"{self.name}_num_leaves", 2, 200, default_value=10
        #     ),
        #     ConfigSpace.UniformFloatHyperparameter(
        #         f"{self.name}_feature_fraction", 0.4, 1.0, default_value=1.0
        #     ),
        #     ConfigSpace.UniformFloatHyperparameter(
        #         f"{self.name}_bagging_fraction", 0.4, 1.0, default_value=1.0
        #     ),
        #     ConfigSpace.UniformIntegerHyperparameter(
        #         f"{self.name}_bagging_freq", 0, 7, default_value=0
        #     ),
        #     ConfigSpace.UniformIntegerHyperparameter(
        #         f"{self.name}_min_child_samples", 5, 100, default_value=20
        #     ),
        #     ConfigSpace.UniformFloatHyperparameter(
        #         f"{self.name}_reg_alpha", 0.0001, 10.0, default_value=0.0001, log=True
        #     ),
        #     ConfigSpace.UniformFloatHyperparameter(
        #         f"{self.name}_reg_lambda", 0.0001, 10.0, default_value=0.0001, log=True
        #     )
        # ))
