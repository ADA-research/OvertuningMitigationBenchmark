...
for constructing the hyperparameter search space for the MLPRegressor
model using ConfigSpace. All hyperparameters added to this space
are conditional on the model being set to 'MLPRegressor'.

Classes:
    - MLPSpace: Constructs the hyperparameter space for MLPRegressor.
"""
...
class MLPSpace(BaseComponentSpace):
    """
    Defines the configuration space for the MLPRegressor model.

    This class builds the search space for fitting an MLPRegressor model
    ...
    def __init__(self, component_hp, seed=0):
        """
        Initializes the MLPRegressor hyperparameter search space.

        Args:
            component_hp (ConfigSpace.hyperparameter.Hyperparameter):
                The top-level hyperparameter selector that identifies the model
                type (e.g., 'MLPRegressor').
        """
